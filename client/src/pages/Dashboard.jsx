...
const Dashboard = () => {
    ...
    const deleteResume = async (resumeId) =>  {      // to show a pop up window if we really want to delete resume
...
<p className='text-2xl font-medium mb-6 bg-gradient-to-r from slate-600 to slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'> Welcome,Raj </p>
    ...
    <div className='flex gap-4'>
     ...
     text-slate-600 border border-dashed border-slate-300 group hover:border-green-lg transition-all duration-300 cursor-pointer'>
    ...
    <button onClick={()=> setShowUploadResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2
     text-slate-600 border border-dashed border-slate-300 group hover:border-purple-lg transition-all duration-300 cursor-pointer'>
    ...
    <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
        ...
        {allResumes.map((resume,index)=>{
            ...
            return(
                ...
                gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{
                    background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor:baseColor + '40'}}>
                     ...
                     Updated on {new Date(resume.updatedAt).toLocaleDateString()}
                       ...
                       <div onClick={e=> e.stopPropagation()} className='absolute top-1 right-1 group-hover:flex items-center hidden'>
                        ...
                        <PencilIcon onClick={()=> {setResumeId(resume._id); setTitle(resume.title)}} className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors"/>
                       </div>
...
{editResumeId && (
          <form onSubmit={editTitle} onClick={()=> setResumeId (false)}
              className ='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
